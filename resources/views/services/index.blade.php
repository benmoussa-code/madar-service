                ...
                <div class="w-full lg:w-1/4">
                    <div class="bg-white/80 backdrop-blur-xl p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/40 sticky top-24">
                        <div class="flex items-center justify-between mb-8">
                            <h2 class="text-xl font-bold text-slate-900 tracking-tight">{{ __('Filtrer') }}</h2>
                        ...
                        <form action="{{ route('services.index') }}" method="GET" class="space-y-8">
                            <!-- Search Section -->
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-4">{{ __('Recherche') }}</label>
                            ...
                            <!-- Categories Section -->
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-4">{{ __('Catégories') }}</label>
                ...
                <div class="w-full lg:w-3/4">
                    <div class="mb-10 flex flex-col md:flex-row justify-between md:items-end gap-6">
                        <div class="space-y-1">
                            ...
                            <p class="text-slate-400 text-sm font-medium">{{ __("Trouvez l'expertise dont vous avez besoin aujourd'hui.") }}</p>
                        </div>
                        <div class="inline-flex items-center px-4 py-2 rounded-xl bg-blue-50 border border-blue-100 text-[11px] font-bold text-blue-600 uppercase tracking-widest">
                            <span class="mr-2">{{ $services->total() }}</span> {{ __('Services') }}
                        </div>
                    </div>
                    ...
                    @if($services->isEmpty())
                        <div class="bg-white p-20 rounded-[2.5rem] text-center shadow-sm border border-slate-100">
                            ...
                            <h2 class="text-2xl font-bold text-slate-900 mb-2">{{ __('Aucun service trouvé') }}</h2>
                            <p class="text-slate-400 text-sm max-w-xs mx-auto">{{ __("Essayez d'ajuster vos filtres ou effectuez une nouvelle recherche.") }}</p>
                        </div>
                    @else
